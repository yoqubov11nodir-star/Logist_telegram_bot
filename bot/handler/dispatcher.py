...
STATUS_UZ = {
    ...
    "DIDOX_TASDIQDA":      "✅ Ruxsat berildi — tushirish",
...
async def view_assigned_orders(message: Message, user: User):
    ...
    async with async_session() as session:
        ...
        # Logist va client ma'lumotlarini olish
        logist_ids = list({o.logist_id for o in orders if o.logist_id})
        users_map = {}
        if logist_ids:
            ulist = (await session.execute(select(User).where(User.telegram_id.in_(logist_ids)))).scalars().all()
    ...
    for order in orders:
        ...
        logist_u = users_map.get(order.logist_id)
...
async def dispatcher_my_orders(message: Message, user: User):
    ...
    async with async_session() as session:
        ...
        # Kerakli user IDlarni yig'amiz
        uid_set = set()
        for o in active_orders:
            if o.logist_id:  uid_set.add(o.logist_id)
...
async def dispatcher_done_orders(callback: CallbackQuery, user: User):
    ...
    for o in chunk:
        ...
        profit   = sale - cost
        date_str = o.created_at.strftime("%d.%m.%Y") if o.created_at else "—"
        c_str = o.client_name or "—"
        await callback.message.answer(
            f"━━━━━━━━━━━━━━━━━━\n"
            f"📦 <b>#{o.id}</b> — {status_name} | 📅 {date_str}\n"
            f"━━━━━━━━━━━━━━━━━━\n"
            f"👤 Mijoz: {c_str} | 📱 {o.client_phone}\n"
...
async def list_drivers(callback: CallbackQuery):
    ...
    kb = InlineKeyboardBuilder()
    for d in drivers:
        phone_str = d.phone or "tel yo'q"
        card_str  = f" | 💳{d.card_number}" if d.card_number else ""
        kb.button(
            text=f"🚛 {d.full_name} | 📱{phone_str}{card_str}",
            callback_data=f"ask_vnum_{order_id}_{d.id}"
        )
    kb.button(text="➕ Yangi haydovchi qo'shish", callback_data=f"add_new_driver_{order_id}")
    kb.adjust(1)
    await callback.message.edit_text(
        f"🚛 <b>#{order_id} uchun haydovchi tanlang:</b>",
        reply_markup=kb.as_markup(), parse_mode="HTML",
    )
...
async def assign_driver_to_order(message: Message, state: FSMContext):
    ...
    async with async_session() as session:
        ...
        driver = (await session.execute(select(User).where(User.id == driver_db_id))).scalar_one_or_none()
        if not order or not driver:
            await message.answer("❌ Buyurtma yoki haydovchi topilmadi. Sahifani yangilang.")
            await state.clear()
            return
    ...
    try:
        ...
        a_loc_status = "✅ quyida yuborildi" if a_lat and a_lon else "❌ kiritilmagan"
        b_loc_status = "✅ quyida yuborildi" if b_lat and b_lon else "❌ kiritilmagan"
            ...
            try:
                await callback.bot.send_message(
                    ...
                    text=(
                        ...
                        f"📱 Tel: {driver.phone if driver and driver.phone else '—'}\n"
...
async def process_rejection_reason(message: Message, state: FSMContext):
    ...
    if driver:
        stage_text = {
            "loading_a":   "yuk ortish",
            "unloading_b": "yuk tushirish",
            "b_arrival":   "B nuqtaga kelish"
        }.get(stage, stage)
        try:
            await message.bot.send_message(
                chat_id=driver.telegram_id,
...
# ─── MIJOZ UCHUN LOKATSIYA YUBORISH ──────────────────────────────────────────
@dispatcher_router.callback_query(F.data.startswith("send_loc_to_client_"))
async def send_location_to_client(callback: CallbackQuery):
    order_id = int(callback.data.split("_")[4])
    async with async_session() as session:
        order = (await session.execute(select(Order).where(Order.id == order_id))).scalar_one_or_none()
        if not order:
        ...
        client = (await session.execute(select(User).where(User.phone == order.client_phone))).scalar_one_or_none()
        driver = (await session.execute(select(User).where(User.telegram_id == order.driver_id))).scalar_one_or_none()
        latest_loc = (await session.execute(
    ...
    if not latest_loc:
        await callback.answer("Haydovchi hali lokatsiya yubormagan!", show_alert=True)
        return
    ...
    try:
        ...
        await callback.bot.send_message(
            ...
            text=(
                f"🚚 <b>#{order_id} — Yukingiz yo'lda!</b>\n\n"
                f"📍 Yuqoridagi — haydovchining hozirgi joylashuvi\n\n"
                f"━━━━━━━━━━━━━━━━━━\n"
                f"👨‍✈️ Haydovchi: {driver.full_name if driver else '—'}\n"
                f"📱 Tel: {driver.phone if driver and driver.phone else '—'}\n"
                f"🚘 Mashina: {order.vehicle_number or '—'}\n"
        ...
        await callback.answer("✅ Mijozga lokatsiya yuborildi!")
    except Exception as e:
        logging.error(f"Clientga lokatsiya: {e}")
...
async def dispatcher_stats(message: Message, user: User):
    ...
    async with async_session() as session:
        orders = (await session.execute(
            select(Order).where(Order.dispatcher_id == user.telegram_id)
        )).scalars().all()
    total     = len(orders)
