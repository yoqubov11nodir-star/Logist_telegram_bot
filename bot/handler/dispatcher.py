...
async def list_drivers(callback: CallbackQuery):
    ...
    await callback.message.edit_text("Ushbu yuk uchun haydovchini tanlang:", reply_markup=kb.as_markup())
...
async def ask_vehicle_number(callback: CallbackQuery, state: FSMContext):
    ...
    await state.update_data(temp_order_id=int(order_id), temp_driver_id=int(driver_id))
    
    await callback.message.answer("🚛 Iltimos, mashina davlat raqamini kiriting (masalan: 01A777BA):")
...
async def assign_driver_to_order(message: Message, state: FSMContext):
    ...
    async with async_session() as session:
        ...
        driver_res = await session.execute(select(User).where(User.id == driver_id))
    ...
    await message.answer(f"✅ Buyurtma #{order_id} haydovchi {driver.full_name}ga ({v_number}) biriktirildi!")
    ...
    if client:
        try:
            await message.bot.send_message(
                ...
                text=f"🚛 Buyurtmangiz #{order_id} uchun haydovchi biriktirildi.\n🚘 Mashina raqami: {v_number}\n👨‍✈️ Haydovchi: {driver.full_name}"
            )
        except: pass
...
# --- YUK TUSHIRISHNI TASDIQLASH (APPROVE UNLOADING B) ---
...
async def approve_unloading_b(callback: CallbackQuery):
    ...
    await callback.message.edit_text(f"✅ #{order_id} buyurtma: Yuk tushirilishi tasdiqlandi. Kassirga yuborildi.")
    ...
    await callback.bot.send_message(
        chat_id=driver.telegram_id,
        text=f"✅ Yuk tushirilishi tasdiqlandi. To'lov kutilmoqda..."
    )
